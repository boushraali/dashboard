    ...
    public function SaveItems(Request $request)
{
  $data = items::create([
    ...
    'color' => $request->color,
    'image' => $request->image,
    'itemgroupno' => $request->itemgroupno, // Ensure to include this line
]);

    // You don't need to call $data->save() explicitly here.

    return redirect('items')->with('success', 'تم الحفظ بنجاح');
}

#delet item 

public function delet($x)
    {
      $item=items::find($x);
...
public function Updated(Request $request)
    ...
    public function Edited($x)
    {
      $item=items::where('id',$x)
      ->first();
      return view('edititem',['items'=>$item]);
